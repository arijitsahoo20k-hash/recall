...
3. Go to **SQL Editor** → paste and run the full contents of `supabase/schema.sql` — this also creates the `avatars` storage bucket and its upload/update/delete policies, so profile photo uploads work out of the box.
   - If this step fails specifically on the storage policy statements with `must be owner of table objects`, that's a known platform-side restriction on some projects (not something the SQL can control) — create the bucket and policies manually instead via **Storage** → New bucket (`avatars`, public), then **Storage → Policies** → New Policy, using the same conditions shown in the `STORAGE: AVATARS BUCKET` section of `schema.sql`.
...
- ✅ Plan Revisions — add or re-generate a revision schedule for a chapter any time after it's created, not just at creation time
