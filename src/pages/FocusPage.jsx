...
export default function FocusPage() {
  ...
  const tick = useCallback(() => {
    ...
    // BUG FIX: derive elapsed from actual wall-clock time (Date.now() minus
    // the session's real start time) instead of incrementing a counter by
    // 1 per tick. setInterval ticks can be delayed or skipped — e.g. when
    // the browser throttles a backgrounded tab — which would otherwise
    // make the displayed elapsed time silently fall behind real time.
  ...
  function playDing() {
    ...
    } catch (_) { /* AudioContext unavailable or blocked — sound is non-critical */ }
  }
        ...
        {phase === 'complete' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', width: '100%', maxWidth: 360 }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button className="btn btn-secondary btn-lg" style={{ flex: 1 }} onClick={() => { setPhase('setup'); setElapsed(0); setTimeLeft(selectedDuration); startTimeRef.current = null }}>
                Discard
              </button>
