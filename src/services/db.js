...
export const revisionService = {
  ...
  // Lets a user plan a single revision date by hand, any time after a
  // chapter already exists — not just at chapter-creation time via the
  // auto-generate checkbox. Always tagged note_type='manual' so it never
  // collides with auto-generated rows in the unique index that backs
  // ON CONFLICT DO NOTHING in generate_spaced_revisions/generate_formula_revisions.
  async createManual(userId, chapterId, scheduledDate, notes = '') {
    const { data, error } = await supabase
      .from('revisions')
      .insert({
        user_id: userId,
